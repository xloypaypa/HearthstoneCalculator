...
public class CardListBuilderTest {
    ...
    @Test
    public void should_build_3_cards_with_1_2_3_cost_when_give_1_1_1_in_cardCosts() {
        CardListBuilder cardListBuilder = new CardListBuilder();
        Card[] cards = cardListBuilder.buildCardList(new int[]{0, 1, 1, 1}, 0);
    ...
    public void should_build_1_unusable_cards_when_give_1_other() {
        CardListBuilder cardListBuilder = new CardListBuilder();
        Card[] cards = cardListBuilder.buildCardList(new int[]{0, 1, 1, 1}, 1);
        Set<Integer> costSet = new HashSet<>();
        for (Card now : cards) {
            costSet.add(now.getCost());
        }
    ...
    public void should_get_InvalidParameterException_when_give_negative_number_in_cardCosts() {
        CardListBuilder cardListBuilder = new CardListBuilder();
        cardListBuilder.buildCardList(new int[]{0, 1, 1, -1}, 0);
    }

    @Test(expected = InvalidParameterException.class)
    public void should_get_InvalidParameterException_when_give_negative_number_in_other() {
        CardListBuilder cardListBuilder = new CardListBuilder();
        cardListBuilder.buildCardList(new int[]{0, 1, 1, 1}, -1);
    }
